...
var AreaGraphSize=[];
var Area = function(level0, level1, level2, level3, containerId, colorOptions, localJsonData, charttitle, showlegends, showtooltip, alignment, tooltipgroup, valueby,xaxistitle,yaxistitle) {
  var graphWidth =0;
  var graphHeight =0;
  if(typeof AreaGraphSize[containerId] =='undefined') {
     var $container = $('#'+containerId);
     var size =  { width : $container.width(), height:  $container.height()};
     AreaGraphSize[containerId]= size;
     graphWidth = $container.width();
     graphHeight = $container.height();
   }
   else {
    if(AreaGraphSize[containerId]) {
     graphWidth= AreaGraphSize[containerId].width;
     graphHeight = AreaGraphSize[containerId].height;
	
    }
   }
    var typeofobject = typeof colorOptions;
    if (typeofobject == 'string') {
        Area.colorOptions = JSON.parse(colorOptions)
    } else {
        Area.colorOptions = colorOptions;
    }
    ...
    xhttp.onreadystatechange = function() {
        if (this.readyState == 4 && this.status == 200) {
            //Area.responseobj = JSON.parse(this.response);
            var responseData = JSON.parse(this.response);
			var graphData = Area.getReponseData (responseData,level0);
		    Area.responseobj = graphData.finalData;
		    level0= graphData.level0;
			Area.group0=level0;
            var output = Area.creategrouping();
...
Area.creategrouping = function(selectedgroup) {
    var responsedataArr = Area.jsonPath(Area.responseobj, "$." + Area.group0);
    var data = { data: responsedataArr[0] };
    var level1arr = Area.jsonPath(data, "$.." + Area.group1);
    //var level1arr = Area.jsonPath(Area.responseobj, "$.." + Area.group1);
    var output = [];
    ...
    if (level1arr) {
        ...
        switch (typeof content) {
            ...
            case "string":
                {
                    if (selectedgroup) {
                        switch (Area.levelindex) {
                            case 0:
                                {
                                    output = Area.getmainlevel(level1arr);
                                }
                                break;
                            case 1:
                                {
                                    output = [];
                                    //var arrAccsummary = Area.responseobj['ACCOUNT_SUMMARY'];
                                    var arrAccsummary = Area.responseobj[Area.group0];
                                    var count = 0;
                                    for (var i = 0; i < arrAccsummary.length; i++) {
                                        var obj = arrAccsummary[i];
                                        var value = obj[Area.group2];

                                        if ((obj[Area.group1] == selectedgroup) && value) {
                                            Area.identifyColor(Area.colorOptions, value, count, output, Area.group2, output.length);
                                            count = count + 1;
                                        }
                                    }
                                }
                            ...
                            case 2:
                                {
                                    ...
                                    for (var i = 0; i < arrAccsummary.length; i++) {
                                        var obj = arrAccsummary[i];
                                        var value = obj[Area.group3];

                                        if ((obj[Area.group2] == selectedgroup) && value) {

                                            Area.identifyColor(Area.colorOptions, value, count, output, Area.group3, output.length);
                                            count = count + 1;
                                        }
                                    }
                                }
...
Area.getmainlevel = function(level1arr) {
    var data = [];
    var output = [];
    for (var i = 0; i < level1arr.length; i++) {
        if (!data[level1arr[i]]) {
    ...
    Object.keys(data).forEach(function(key) {
        var value = data[key];
        Area.identifyColor(Area.colorOptions, key, count, output, key, count);
        count = count + 1;
    });
    return output;
}
Area.identifyColor = function(colorOptions, value, count, output, key, times) {
    var nocolor = true;
            ...
            if (colorvalue) {
                ...
                if (legendvalue) {
                    output.push({
                        ...
                        times: times,
                        index: count,
                        color: Area.getrandomcolor(count)
                    });
                } else {
                    output.push({
                        value: key,
...
Area.getrandomcolor = function(index) {
...
// Area.draw = function(data, containerId, charttitle,chartfor,yaxistitle) {

// 	//isolate the data for legends
//     var dataarr = [];
//     var titles = data.map(obj => obj.value);
//     var values = data.map(time => time.times);
...
// }
Area.draw = function(data, containerId, charttitle, showlegends, showtooltip, alignment, tooltipgroup, valueby,graphWidth,graphHeight,xaxistitle,yaxistitle) {
  ...
  var width=0;
  var height=0;
  if(typeof graphWidth =='undefined'&&AreaGraphSize[containerId])
  {
   graphWidth= AreaGraphSize[containerId].width;
   graphHeight = AreaGraphSize[containerId].height;
  }
   ...
   height = graphHeight + margin.left + margin.right;
    ...
    circle.on("mouseover", function(d) {
        ...
        .on("click", function(d) {
            if (Area.levelindex < 2)
                Area.levelindex += 1;
            else
                Area.levelindex = 0;

            var output = Area.creategrouping(d.value);
            if (output.length > 0) {
                Area.draw(output, containerId, charttitle, showlegends, showtooltip, alignment, tooltipgroup, valueby,graphWidth,graphHeight,xaxistitle,yaxistitle);
            } else {
                var level1arr = Area.jsonPath(Area.responseobj, "$.." + Area.group1);
                if(Area.group2.length > 0){
                    output = Area.getmainlevel(level1arr);
                    Area.draw(output, containerId, charttitle, showlegends, showtooltip, alignment, tooltipgroup, valueby,graphWidth,graphHeight,xaxistitle,yaxistitle);
                }
            }
    ...
    Area.addlegends(svg, data, donutwidth, donutheight, showlegends, alignment);

}
Area.addlegends = function(svg, data, width, height, showlegends, alignment) {
...
Area.jsonPath = function(obj, expr, arg) {
    var P = {
...
Area.getReponseData = function(resData, lev0) {
